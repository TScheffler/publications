...
So, Kukich (\cite{kukich}), for instance, divided all NTN techniques
into six classes:
\begin{enumerate}
...
etc. With regard to the type of information inference, practically all
of these methods were supervised with the exceptions of Cook and
Stevenson (\cite{cook}) who claimed to use an unsupervised technique,
and Sproat et al. (\cite{sproat}) who described both a supervised and
...
A striking outlier of 16.22~\% for slang tokens in column 1 is
explained by the fact that the word ``RT'' which occurred 1,235 times
in our data and was by far the most frequent OOV in analyzed texts,
was recognized as out-of-vocabulary by \texttt{hunspell} but was not
deemed as such by \texttt{TreeTagger}. Luckily, such singleton cases
were rather rare exceptions during the analyses and did not affect
much the remaining classes of OOVs, so that distributions of unknown
tokens for both tools were more or less similar or at least comparable
to a certain extent.
...
a half of all unknown words found in data. Since different classes of
these Twitter phenomena were formed by different processes and also
have different degrees of ambiguity, we will suggest different
normalization procedures for each of them in the next Section.

\section{Text Normalization Procedure}\label{normalization}

\subsection{Replacement of Twitter-Specific Phenomena}
Though hashtags are almost ubiquitous in today's tweets and one would
probably think that they are as old as Twitter itself, an admirable
truth would be that hashtags actually existed long before
Twitter-service was introduced. It were Internet Relay Chats (IRC) --
the forebears of all modern social networks -- which used the hash
sign to mark channel names. And it was Chris Messina, a renowned
advocate of open-source community, who borrowed this idea and brought
it to Twitter. The ``hash godfather'', as Chris Messina describes
himself, sent out a Twitter message suggesting that the pound sign
coul be used for organizing groups on Twitter in August 2007. But a
noticeable dissemination of this ``novelty'' among the Twitter
community only happened a few years later, in 2010. ``I would say 2010
was really the year of the hashtag.'' claims Ginger Wilcox, a founder
of the Social Media Marketing Institute (cf. Parker, \cite{parker}).

Luckily for us, both hashtags and @-names which are another
omnipresent phenomenon in Twitter have very clear formal criteria by
which these tokens could be identified and captured. The existence of
such formal cues suggests that these phenomena could best be handled
by means of rule-based methods. A most appropriate of such methods
seemed to be an analog of finite-state transducers (FST).
