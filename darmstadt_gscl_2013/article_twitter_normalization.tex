...
\begin{enumerate}
\item regular German words, e.g. \textit{meins},
...
Luckily for us, this ``novelty'' by Messina brought along a strict
formal feature by which future hashtags could be identified. Other
types of words which are usually considered as OOVs in Twitter but
also have some unambiguous formal traits include at-tokens,
hyperlinks, e-mail addresses, and smileys. The presence of such formal
criteria suggests that these classes could best be handled by
rule-based methods and namely finite-state techniques like
finite-state transducers (FST) (cf. Jurafsky and Martin,
\cite{jurafsky}, pp.57-60).
...
corresponding actions performed on matched subgroups in cases when
these subgroups did not overlap. If some rules clashed together on
certain contexts, a set of special criteria was used to determine
which rule the preference should be given.

In this system, unambiguous tokens like web links and e-mail addresses
...
played a certain syntactic role in a sentence or could be easily
...
of all replacements we remembered along with original words which were
replaced and a restoration step to original forms was performed for
certain classes of tokens after tagging.
