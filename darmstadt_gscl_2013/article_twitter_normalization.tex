...
Based on these two classifications, recently proposed approaches to
...
supervised with the exceptions of Cook and Stevenson (\cite{cook}) who
claimed to use an unsupervised technique, and Sproat et
al. (\cite{sproat}) who described both a supervised and an
...
unique OOVs disregarding the frequencies (column ``\uniqoov{}'').
\begin{table}
...
relatively frequent omissions of characters made by users or even more
often automatic truncation of too long messages which is performed by
Twitter service itself.\footnote{As is generally known, Twitter
...
\end{enumerate}
according to the formal or lexical class which these tokens belonged
...
corresponding colloquial equivalents like, for example, \textit{ned}
instead of \textit{nicht} or \textit{grad} instead of \textit{gerade}.
